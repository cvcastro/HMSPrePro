...
""" Snyder """
def Snyder():
    arcpy.AddField_management(basin, "Cp", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")
    arcpy.AddField_management(basin, "Ct", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")
    arcpy.AddField_management(basin, "Tp", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")

    with arcpy.da.UpdateCursor(basin, ["LONGST_FT", "Cp", "Ct", "Tp"]) as cursor:
        for row in cursor:
            row[1] = peaking
            row[2] = steepness
            row[3] = round(math.pow(float(row[2])*(float(row[0])/5280*float(row[0])/5280/2),0.3),2)
            cursor.updateRow(row)
    del row, cursor
...
def snyderempty():
    ...
    with arcpy.da.UpdateCursor(basin, ["Cp", "Ct", "Tp"]) as cursor:
            for row in cursor:
                row[0] = 0
                row[1] = 0
                row[2] = 0
...
def TR55fields(longest):
    ...
    arcpy.AddField_management(longest, "BW", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")
    arcpy.AddField_management(longest, "Depth", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")

    with arcpy.da.UpdateCursor(longest, ["L_sh", "Precip2yr", "Surface", "n_ch", "n_ol", "BW", "Depth"]) as cursor:
        for row in cursor:
            row[0] = sheet
            row[1] = precip
            row[2] = surface
    ...
    with arcpy.da.UpdateCursor(longest, ["R", "BW", "Depth"]) as cursor:
        for row in cursor:
            row[0] = (row[1]*row[2]) / (row[1]+row[2]*2)
            cursor.updateRow(row)
    del row, cursor
...
if str(CBsnyder) == "true":
    arcpy.AddMessage("Calculating Snyder parameters...")
    Snyder()
else:
    snyderempty()
